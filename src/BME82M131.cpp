...
uint16_t BME82M131::readALS(uint8_t sensornumber)
{
  ...
  delay(15);
   if(readBytes(_rx_buf,7))
   {
    ...
    {
      uint16_t als=0;
      als = (u16)_rx_buf[4] + ((u16)_rx_buf[5] << 8);
      return als;
    }
    else
    {
...
{  
  ...
  if(readBytes(_rx_buf,7))
  {
    ...
    if(status == 0)
    {
      white = (u16)_rx_buf[4] + ((u16)_rx_buf[5] << 8);
     return white;
    }
    else
    {
...
uint8_t BME82M131::getInterruptFalg(uint8_t sensorNmuber)
{
  ...
  if(readBytes( _rx_buf,7))
  {
    ...
    if(status == 0)
    {
      REG_Temp.all = (u16)_rx_buf[4] + ((u16)_rx_buf[5] << 8);
...
uint16_t BME82M131::getHighThreshold(uint8_t sensorNmuber)
{
  ...
  { 
    if( _rx_buf[3] == 0)
    {
      threshold = (u16)_rx_buf[4] + ((u16)_rx_buf[5] << 8);
      return threshold;
    }
    else
    {
...
bool BME82M131::getPsmReg(uint8_t sensornumber, uint16_t &psm)
{
  ...
  if(readBytes(_rx_buf,7))
  {
    ...
    if(status == 0)
    {
      psm = (u16)_rx_buf[4] + ((u16)_rx_buf[5] << 8);
      return true;
    }
    else
    {
...
bool BME82M131::getAlsConfig(uint8_t sensornumber,uint16_t &als_conf)
{
  ...
  if(readBytes(_rx_buf,7))
  {
    ...
    if(status==0)
    {
      als_conf = (u16)_rx_buf[4] + ((u16)_rx_buf[5] << 8);
      return  true;
    }
    else
    {
